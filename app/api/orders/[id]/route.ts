...
type OrderUpdate = { status: string; updated_at: string; completed_at?: string };
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const update: OrderUpdate = { status: parsed.data.status, updated_at: new Date().toISOString() };
