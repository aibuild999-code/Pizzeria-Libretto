...
export async function GET() {
  try {
    ...
    const { data: categories, error: categoryError } = await supabase.from("menu_categories").select("id,name,display_order,is_active").eq("restaurant_id", restaurant.id).eq("is_active", true).order("display_order");
    if (categoryError) throw categoryError;
    const categoryIds = (categories ?? []).map((c) => c.id);
    const { data: items, error: itemError } = categoryIds.length ? await supabase.from("menu_items").select("id,category_id,name,description,price,dietary_tags,is_available,display_order,image_url,item_type").in("category_id", categoryIds).order("display_order") : { data: [], error: null };
    ...
    for (const result of [sizes, ingredients, itemIngredients, itemGroups, groups, modifiers, sizeRules, itemAvailability, comboGroups, comboOptions, quantityLevels]) if (result.error) throw result.error;
    ...
    const enrichedItems = ((items ?? []) as AnyRow[]).map((item) => ({
      ...
      modifier_groups: itemGroupRows.filter((link) => link.menu_item_id === item.id).map((link) => ({
        ...
        modifiers: modifierRows.filter((m) => m.modifier_group_id === link.modifier_group_id).map((m) => ({ ...m, quantity_levels: quantityRows.filter((q) => q.modifier_id === m.id) })),
        size_rules: sizeRuleRows.filter((rule) => rule.menu_item_id === item.id && rule.modifier_group_id === link.modifier_group_id),
      })),
      combo_groups: comboGroupRows.filter((cg) => cg.combo_item_id === item.id).map((cg) => ({ ...cg, options: comboOptionRows.filter((option) => option.combo_group_id === cg.id) })),
    }));

    const grouped = (categories ?? []).map((category) => ({ ...category, menu_items: enrichedItems.filter((item) => item.category_id === category.id) }));
